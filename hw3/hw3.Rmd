...
Below, we can see that during a recession, the proposed rate is either consistently over or consistently under the actual Fed Funds rate. This difference doesn't vary dramatically between recession and recovery.
...
for (i in seq_along(recessions$Peak)){
  if (i < length(recessions$Peak)){
    econ[ (econ$DATE >= recessions$Peak[i] & econ$DATE < recessions$Peak[i+1]), ]$ERA <- i
  }
  else {
    econ[ (econ$DATE >= recessions$Peak[i]), ]$ERA <- i
  }
}
...
```{r weeks_since}
week_plot <- ggplot(data = econ, aes(x=WEEKS_SINCE_REC)) +
        geom_point(aes(y=DIFF, color=DATE)) + 
        scale_color_gradientn(colours=pallete2) + 
        geom_hline(yintercept = 0) + 
        labs(y = "Diff (TR - FF)",
             x = "Weeks since recession",
             title="Rate Difference is Constant Over Recession Periods",
             color="Days from 1960",
...
#stack the eras
week_slice_plot <- ggplot(data = econ) +
                    # need a color scale!
                    geom_hline(yintercept = 0) + 
                    labs(y = "Diff (TR - FF) %",
                         x = "Weeks Since recession",
                         color= "Recession Era",
                         title="Rate Difference slowly declines in more stable periods",
                         subtitle="Difference in rates over distance in weeks since a recession",
                         caption="source: FRB St. Louis")
...
for (i in seq_along(recessions$Peak)){
  ...
  week_slice_plot <- week_slice_plot + geom_line(data=new_slice, aes(x=WEEKS_SINCE_REC, y=DIFF), color=i)
}
week_slice_plot
```
