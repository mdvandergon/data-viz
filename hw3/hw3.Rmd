...
theme_mark <- function(base_size=12, font=NA){
  ...
  txt <- element_text(size = base_size, color = "black", family="Nexa Light")
  bold_txt <- element_text(size = base_size+2, color = "black", family="Nexa Bold")
...
diff <- ggplot(data = econ, aes(y=DIFF)) +
      geom_point(aes(x=DATE, color=DATE), show.legend = FALSE) +
      geom_hline(yintercept = 0) + 
      labs(y = "Diff (TR - FF)",
           x = "Date",
           title="Taylor Over Estimates, and slightly more when changes in GDP are Higher",
           subtitle="Difference in TR and Fed Funds over time (source: FRB St. Louis)")
diff
```
...
diff <- ggplot(data = econ, aes(y=DIFF)) +
      ...
      labs(y = "Diff (TR - FF)",
           ...
           title="Recently, Taylor Rule has Over Estimated and GDP less Volatile",
